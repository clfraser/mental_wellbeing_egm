  ...
  reviews_chart %>%
    mutate(selected = 0,
           selected = if_else(  (dummy == 0 &
                                ...
                                overall_outcome %in% c("Outcome category 2", "Outcome category 3"),
                              1,
                              0))
}, ignoreNULL = FALSE)

only_selected <- reactive({filtered() %>%
  filter(selected == 1)})
  
output$egm <- renderReactable({
    ## create EGM plot    
    grouped <- reviews_table %>%
      filter(study_id %in% only_selected()$study_id) %>%
      separate_longer_delim(intervention_or_exposure, delim = "; ") %>%
      separate_longer_delim(overall_domain, delim = "; ") %>%
      separate_longer_delim(subdomain, delim = "; ") %>%
      group_by(overall_domain, subdomain, overall_outcome, intervention_or_exposure) %>%
      summarise(count = n()) %>%
      ungroup()
    ...
    count_pivot <- grouped %>%
      ...
      pivot_wider(names_from = c(overall_outcome, intervention_or_exposure), values_from = count, names_sep = ".") %>%
      mutate(across(Self_harm.Exposure:Outcome_category_3.Intervention, ~replace_na(., 0))) # Replace NAs with 0
    #%>%
      #select(-Self_harm.NA)
    ...
    count_pivot %>%
      reactable(
        defaultColDef = colDef(
          align = 'center',
          maxWidth = 150),
        groupBy = "overall_domain",
        defaultExpanded = TRUE,
        onClick = JS("function(rowInfo, column) {
        // Don't handle click events in the overall domain or subdomain columns
        if (column.id === 'overall_domain' || column.id === 'subdomain') {
          return
        }
