...
#df <- df %>% 
#mutate(exposure_definition = str_pad(exposure_definition,max_exp_char,"both"),
#       outcome_definition = str_pad(outcome_definition,max_exp_char,"both"))
...
# Create a column to code for exposure, intervention or both
df_pivot <- df_source %>%
  ...
  mutate(domain = to_sentence_case(domain),
         intervention_exposure_short = case_when(str_detect(intervention_or_exposure, "exposures") & str_detect(intervention_or_exposure, "Intervention") ~ "Both",
...
# Create columns for each outcome definition and subdomain, since some studies have several
...
df_separated <- df_pivot %>%
  ...
  mutate(subdomain = case_when(str_detect(subdomain,"Other:") ~ "Other",
...
df_separated <- df_separated %>%
  ...
  add_row(overall_outcome = "Outcome category 2", outcome_definition = "Outcome 4", domain = "Individual", subdomain = "Mental health", intervention_exposure_short = "Both", review_type = "Quantitative") %>%
...
df_separated <- df_separated %>%
  mutate(pos_y = if_else(intervention_exposure_short %in% c("Both", "Intervention"), 1, 2),
         pos_x = case_when(intervention_exposure_short == "Both" ~ 1,
